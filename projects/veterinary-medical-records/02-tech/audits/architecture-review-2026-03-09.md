---
title: "Architecture Review — 2026-03-09"
type: reference
status: active
...
- [Architecture Review — 2026-03-09](#architecture-review--2026-03-09)
  ...
  - [2. Overall Health Dashboard](#2-overall-health-dashboard)
    - [2.1 Code Health by Category](#21-code-health-by-category)
    - [2.2 Documentation Health by Viewpoint](#22-documentation-health-by-viewpoint)
    - [2.3 Severity Summary (All Phases)](#23-severity-summary-all-phases)
  - [3. Architecture Documentation Audit](#3-architecture-documentation-audit)
    - [3.1 Scope](#31-scope)
    - [3.2 Health Score: 34/100 (CRITICAL)](#32-health-score-34100-critical)
    - [3.3 Key Gaps (17 total)](#33-key-gaps-17-total)
    - [3.4 Anti-Patterns Detected (6)](#34-anti-patterns-detected-6)
    - [3.5 Strengths](#35-strengths)
  - [4. Codebase Audit (9 Categories)](#4-codebase-audit-9-categories)
    - [4.1 Security — 7.8/10](#41-security--7810)
    - [4.2 Build & CI — 9.8/10](#42-build--ci--9810)
    - [4.3 Code Principles — 5.0/10](#43-code-principles--5010)
    - [4.4 Code Quality — 5.0/10](#44-code-quality--5010)
    - [4.5 Dependencies — 9.8/10](#45-dependencies--9810)
    - [4.6 Dead Code — 9.8/10](#46-dead-code--9810)
    - [4.7 Observability — 9.0/10](#47-observability--9010)
    - [4.8 Concurrency — 9.5/10](#48-concurrency--9510)
    - [4.9 Lifecycle — 10/10](#49-lifecycle--1010)
  - [5. Coupling & Hotspot Analysis](#5-coupling--hotspot-analysis)
    - [5.1 Hexagonal Compliance](#51-hexagonal-compliance)
    - [5.2 Critical Hotspots](#52-critical-hotspots)
      - [`review_service.py` — 🔴 CRITICAL (New since Feb audit)](#review_servicepy---critical-new-since-feb-audit)
      - [`candidate_mining.py` — 🔴 CRITICAL](#candidate_miningpy---critical)
    - [5.3 High Hotspots](#53-high-hotspots)
    - [5.4 Regression Pattern](#54-regression-pattern)
  - [6. Evolution Since Feb 2026](#6-evolution-since-feb-2026)
    - [6.1 What Improved (7 categories)](#61-what-improved-7-categories)
    - [6.2 What Regressed (2 categories)](#62-what-regressed-2-categories)
    - [6.3 Feb Decomposition Results](#63-feb-decomposition-results)
  - [7. Risk Matrix](#7-risk-matrix)
  - [8. Recommendations](#8-recommendations)
    - [8.1 Immediate (next sprint)](#81-immediate-next-sprint)
    - [8.2 Short-term (next 2-3 sprints)](#82-short-term-next-2-3-sprints)
    - [8.3 Strategic](#83-strategic)
  - [9. Methodology](#9-methodology)
    - [9.1 Process](#91-process)
    - [9.2 Scoring](#92-scoring)
    - [9.3 Tools Used](#93-tools-used)
    - [9.4 Reproducing This Audit](#94-reproducing-this-audit)
    - [9.5 Companion Documents](#95-companion-documents)

<!-- END doctoc generated TOC please keep comment here to allow auto update -->

---

title: "Architecture Review — 2026-03-09" type: explanation status: active audience: all last-updated: 2026-03-09

---

# Architecture Review — 2026-03-09

> **Comprehensive architecture evaluation of the veterinary-medical-records codebase.** Covers documentation
> completeness, code health across 9 categories, coupling analysis, and hotspot detection. Includes methodology for
> reproducibility and comparison with Feb 23, 2026 baseline.
...
The veterinary-medical-records system has **strong architectural foundations** — clean hexagonal layers, robust
lifecycle management, minimal dependencies, and zero dead code. The Feb 2026 audit's 15 findings drove meaningful
improvements: 7 of 9 code health categories improved, and the 3 monolithic hotspots were successfully decomposed.

However, **two critical concerns** have emerged:

1. **Re-accretion pattern:** Two files born from the Feb decomposition have grown into new hotspots —
   `review_service.py` (1532 LOC, 9 responsibilities, CC 99) and `candidate_mining.py` (1013 LOC, CC 163). Without
   complexity guardrails (CI gates, LOC budgets), decomposition alone doesn't prevent future accumulation.

2. **Documentation gaps in security and deployment:** The architecture documentation scores 34/100 — not because the
   _existing_ documentation is poor (V6 Integration 85/100, V10 Decisions 82/100), but because _entire viewpoints_ are
   missing. There is no security architecture section (V7: 28/100) and no production deployment documentation (V4:
   42/100).
...
_Full details: `tmp/audit/phase1-doc-audit.md`_

### 3.1 Scope

9 documents audited (~3,800 lines): architecture.md, technical-design.md, backend-implementation.md,
frontend-implementation.md, extraction-quality.md, and 4 ADRs. Evaluated against 188-item checklist across 14
viewpoints.

### 3.2 Health Score: 34/100 (CRITICAL)

The low score reflects **documentation completeness**, not documentation quality. What exists is well-organized and
accurate. The score is driven by:

- **V7 Security (28/100):** No security architecture section. Auth is mentioned as "optional bearer-token" scattered
  across 3 documents. No threat model, no encryption strategy, no RBAC design.
- **V4 Deployment (42/100):** Only Docker Compose dev configuration documented. No production deployment strategy,
  scaling plan, or disaster recovery.
...
- **V6 Integration (85/100):** API contracts are detailed with endpoint specs, request/response formats, error codes,
  and conflict semantics.
- **V10 Decisions (82/100):** MADR-format ADRs cover the foundational architecture decisions with clear rationale and
  consequences.
...
_Full details: `tmp/audit/phase2-codebase-audit.md`_
...
**Complexity grade distribution:** 82% of all 317 functions are A/B grade (CC ≤ 10). The problem is concentrated in 7
F-grade and 5 E-grade functions.
...
**Strengths:** Minimal surface (7 backend, 14 frontend). All pinned. No unused deps. All imports verified against
declarations.
...
**Strengths:** 17 loggers via `getLogger(__name__)`. Extraction observability excellent (20-run ring buffer). Health
endpoint. Event taxonomy.
...
**Strengths:** WAL + busy_timeout=5000ms. asyncio.Event for coordination. Task lifecycle management. Thread offloading
for CPU. No global mutable state.
...
_Full details: `tmp/audit/phase3-coupling-hotspots.md`_
...
**Verdict:** Core layers (domain, ports, application) are clean. The 3 violations are pragmatic exceptions, not
structural rot. The most architecturally significant is `infra/scheduler_lifecycle.py → application/processing` which
inverts the dependency direction and should be fixed with a `SchedulerPort` interface.
...
Born from `document_service.py` decomposition. Accumulated review lifecycle, interpretation normalization, microchip
sync, age derivation, visit scoping, text segment extraction, clause normalization, medical record projection, and
weight aggregation — all in one file.

**Decomposition strategy:** VisitAssignmentEngine + SegmentParser/ClauseClassifier + AgeNormalizer +
ReviewPayloadProjector → 4 files, each < 400 LOC.
...
Contains `_mine_interpretation_candidates()` — a 767-LOC function with CC 163 that embeds 25+ field-specific extraction
branches via an inner closure.

**Decomposition strategy:** Per-entity-type FieldCandidateExtractor strategy pattern + CandidateValidator + regex
FieldPattern registry → ~5 files.
...
The Feb audit found 3 monoliths that were decomposed. But **2 of the successor files have already re-accreted into
hotspots**. This pattern suggests:

> **Decomposition without guardrails leads to re-accretion.** LOC budgets, CC gates in CI, and architectural fitness
> functions are needed to sustain the improvement.
...
**Net assessment:** Decomposition worked but successor files need a second round of refactoring (review_service.py,
candidate_mining.py).
...
1. **Create security-architecture.md** — auth strategy, threat model (STRIDE), encryption, rate limiting policy
2. **Create deployment documentation** — production topology, scaling, disaster recovery, backup strategy
3. **Fix infra→application violation** — extract `SchedulerPort` interface; inject via DI
4. **Expose `_shared` functions** — re-export through `application.documents.__init__`
5. **Add structured logging** to review and extraction critical paths (currently 0 log statements)
6. **Add ER diagram** — Mermaid diagram in technical-design.md for 5 core entities
7. **Write missing ADRs** — frontend stack, confidence scoring algorithm (minimum)

### 8.3 Strategic

1. **Prevent re-accretion ADR** — document LOC/CC budgets as architectural decision with CI enforcement
2. **Simplify `extraction_observability/`** — triage.py (CC 64) may be over-engineered
3. **Frontend state management** — extract React Context + useReducer from AppWorkspace
4. **Operational runbooks** — backup, database maintenance, log rotation, failure modes
5. **Metrics collection** — Prometheus/OpenTelemetry for production observability
...
**Documentation (Phase 1):** Viewpoint scores (0-100) based on 188-item checklist. Overall score = weighted average −
severity penalties (capped at -30).

**Code Health (Phase 2):** Per-category scoring formula:
`penalty = (C×2.0) + (H×1.0) + (M×0.5) + (L×0.2); score = max(0, 10 - penalty)`. Normalized to 0-100 for comparison with
Feb baseline.

**Hotspots (Phase 3):** 4-signal scoring (LOC > 500, CC > 10, imports > 8, churn > 8 commits). Files flagged as hotspot
when ≥ 2 signals triggered.
...
1. **Phase 1:** Read all 9 docs in `02-tech/` + `02-tech/adr/`. Apply `architecture-doc-auditor` skill with parameters:
   `audit_depth=exhaustive, include_debt_assessment=true, include_anti_patterns=true`.

2. **Phase 2:** Run security scan (secrets + SQL patterns), TODO scan, dependency verification, import mapping,
   concurrency pattern check, logging count, lifecycle review. Score each of 9 categories using the penalty formula.

3. **Phase 3:** Extract all Python imports, map to hexagonal layers, verify allowed directions. Run
   `radon cc -s -n C backend/app/` for complexity. Run `git log --oneline --since=<baseline_date>` for churn. Score
   files against 4-signal hotspot criteria.
