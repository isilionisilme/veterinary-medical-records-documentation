---
title: "Improvement Backlog — Architecture Review 2026-03-09"
type: reference
status: active
...
- [Improvement Backlog — Architecture Review 2026-03-09](#improvement-backlog--architecture-review-2026-03-09)
  - [Priority Levels](#priority-levels)
  - [Immediate (5 items)](#immediate-5-items)
    - [ARCH-01: Decompose `review_service.py`](#arch-01-decompose-review_servicepy)
    - [ARCH-02: Decompose `candidate_mining.py`](#arch-02-decompose-candidate_miningpy)
    - [ARCH-03: Add CI complexity gates](#arch-03-add-ci-complexity-gates)
    - [ARCH-04: Fix infra-application dependency violation](#arch-04-fix-infra-application-dependency-violation)
    - [ARCH-05: Add structured logging to critical paths](#arch-05-add-structured-logging-to-critical-paths)
  - [Short-term (10 items)](#short-term-10-items)
    - [ARCH-06: Create security architecture documentation](#arch-06-create-security-architecture-documentation)
    - [ARCH-07: Create production deployment documentation](#arch-07-create-production-deployment-documentation)
    - [ARCH-08: Expose `_shared` functions publicly](#arch-08-expose-_shared-functions-publicly)
    - [ARCH-09: Add ER diagram to technical-design.md](#arch-09-add-er-diagram-to-technical-designmd)
    - [ARCH-10: Write missing ADRs](#arch-10-write-missing-adrs)
    - [ARCH-11: Add monitoring/alerting strategy documentation](#arch-11-add-monitoringalerting-strategy-documentation)
    - [ARCH-12: Add capacity planning documentation](#arch-12-add-capacity-planning-documentation)
    - [ARCH-13: Implement production authentication](#arch-13-implement-production-authentication)
    - [ARCH-14: Add content validation for PDF uploads](#arch-14-add-content-validation-for-pdf-uploads)
    - [ARCH-15: Explicitly declare pydantic in requirements.txt](#arch-15-explicitly-declare-pydantic-in-requirementstxt)
  - [Strategic (9 items)](#strategic-9-items)
    - [ARCH-16: Create re-accretion prevention ADR](#arch-16-create-re-accretion-prevention-adr)
    - [ARCH-17: Simplify extraction_observability/ subsystem](#arch-17-simplify-extraction_observability-subsystem)
    - [ARCH-18: Extract frontend state management layer](#arch-18-extract-frontend-state-management-layer)
    - [ARCH-19: Create operational runbooks](#arch-19-create-operational-runbooks)
    - [ARCH-20: Add metrics collection infrastructure](#arch-20-add-metrics-collection-infrastructure)
    - [ARCH-21: Add rate limiting to write endpoints](#arch-21-add-rate-limiting-to-write-endpoints)
    - [ARCH-22: Parameterize PRAGMA table_info call](#arch-22-parameterize-pragma-table_info-call)
    - [ARCH-23: Add configuration reference documentation](#arch-23-add-configuration-reference-documentation)
    - [ARCH-24: Replace wildcard re-export with explicit imports](#arch-24-replace-wildcard-re-export-with-explicit-imports)
...
title: "Improvement Backlog — Architecture Review 2026-03-09" type: explanation status: active audience: all
last-updated: 2026-03-09
...
> Prioritized action items from the [architecture review](architecture-review-2026-03-09.md). Each item includes source
> phase, severity, effort estimate, and acceptance criteria.

**Generated:** 2026-03-09 **Baseline audit:** 2026-02-23 **Total items:** 24 **Distribution:** 5 Immediate, 10
Short-term, 9 Strategic
...
**Problem:** 1,013 LOC, 767-LOC single function (`_mine_interpretation_candidates`), CC 163 (highest in codebase), inner
closure with 25+ field-specific validation blocks.
...
**Problem:** `infra/scheduler_lifecycle.py` imports directly from `application/processing`, inverting the hexagonal
...
**Problem:** 22 log statements across 317 functions (7% coverage). `review_service.py` and `candidate_mining.py` have
zero logging.
...
**Action:** Re-export `_locate_visit_date_occurrences_from_raw_text` through `application/documents/__init__.py`. Update
import in `api/routes_review.py`.
...
**Action:** Add Mermaid ER diagram showing: Document, ProcessingRun, Artifacts, InterpretationVersion, FieldChangeLog.
Include relationship cardinality and foreign keys.
...
**Action:** Document: what to monitor, SLO definitions (availability, error rate, processing latency), alerting rules,
...
**Action:** Document expected data volumes, storage growth projections, SQLite row limits, concurrent processing limits,
...
**Action:** Document architectural decision: maximum file LOC (500), maximum function CC (20), enforcement via CI gates
(ARCH-03). Record rationale based on observed regression pattern.
...
**Action:** Review `triage.py` (CC 64) — `_suspicious_accepted_flags` may be over-engineered. Consider simplifying
classification logic or extracting into data-driven rules.
...
**Action:** Extract AppWorkspace's 15 useState hooks into React Context + useReducer pattern to reduce state hub
concentration.
...
**Action:** Create `03-ops/runbook.md` covering: backup procedures, database maintenance, log rotation, common failure
modes, recovery steps.
...
**Action:** Add basic metrics: request latency histogram, processing duration, document count. Consider Prometheus or
OpenTelemetry.
...
**Action:** Replace `f"PRAGMA table_info({table})"` with assertion that table is in known set, or use allowlisted table
names.
...
**Action:** Add comprehensive configuration reference table to backend-implementation.md covering all env vars and
settings.
...
**Action:** Replace `from backend.app.application.documents import *` in `document_service.py` with explicit named
imports.
